...
function showLoading() {
    if (productsGrid) {
        productsGrid.innerHTML = '<div class="loading" style="text-align: center; padding: 2rem; font-size: 1.2rem;">Loading chronograph watches...</div>';
    }
}
...
async function loadLuxuryWatches(filters = {}) {
    try {
        ...
        console.error('Error loading chronograph watches:', error);
        if (productsGrid) {
...
function displayWatches(watches) {
    ...
    if (watches.length === 0) {
        productsGrid.innerHTML = '<p class="no-products" style="text-align: center; padding: 2rem; color: #666;">No chronograph watches found matching your criteria.</p>';
        return;
    }
