...
function showLoading() {
    if (productsGrid) {
        productsGrid.innerHTML = '<div class="loading" style="text-align: center; padding: 2rem; font-size: 1.2rem;">Loading classic watches...</div>';
    }
}
...
async function loadLuxuryWatches(filters = {}) {
    try {
        ...
        console.error('Error loading classic watches:', error);
        if (productsGrid) {
...
function displayWatches(watches) {
    ...
    if (watches.length === 0) {
        productsGrid.innerHTML = '<p class="no-products" style="text-align: center; padding: 2rem; color: #666;">No classic watches found matching your criteria.</p>';
        return;
    }
